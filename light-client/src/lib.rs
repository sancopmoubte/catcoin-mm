...
pub fn verify_header_json(json: &str, trusted_height: i64, trusted_hash: &str) -> Result<bool, String> {
    ...
    if h.chain_id != "catcoin-candidate-1" { return Err("chain_id 不匹配".into()); }
...
pub fn verify_chain_json(json_blocks: &str) -> Result<usize, String> {
    ...
    for h in &blocks {
        let expected = hash_header(h);
        if h.chain_id != "catcoin-candidate-1" || h.height != height + 1 || (!previous.is_empty() && h.prev_hash != previous) || h.block_hash != expected {
...
#[no_mangle]
pub extern "C" fn catcoin_verify_header(json: *const c_char, trusted_height: i64, trusted_hash: *const c_char) -> i32 {
...
/// 为 Kotlin 演示返回 UTF-8 错误/成功字符串；调用方需调用 catcoin_string_free。
#[no_mangle]
pub extern "C" fn catcoin_verify_header_message(json: *const c_char, trusted_height: i64, trusted_hash: *const c_char) -> *mut c_char {
...
#[no_mangle]
pub extern "C" fn catcoin_string_free(value: *mut c_char) { if !value.is_null() { unsafe { drop(CString::from_raw(value)); } } }
...
mod android_jni {
    ...
    #[no_mangle]
    pub extern "system" fn Java_com_example_catcoinmm_NativeLightClient_verifyHeader(
    ...
    #[no_mangle]
    pub extern "system" fn Java_com_example_catcoinmm_NativeLightClient_verifyHeaderMessage(
        mut env: JNIEnv, _class: JClass, json: JString, trusted_height: i64, trusted_hash: JString
    ) -> jni::sys::jstring {
...
mod tests {
    ...
    fn block(height: i64, prev: &str, app: &str) -> Header {
        let mut h = Header { chain_id: "catcoin-candidate-1".into(), height, time: format!("2026-01-01T00:00:0{height}Z"), prev_hash: prev.into(), app_hash: app.into(), block_hash: String::new() };
